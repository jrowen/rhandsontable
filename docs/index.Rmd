...
rhandsontable(DF, col_highlight = col_highlight, 
  ...
  hot_cols(renderer = "
    function(instance, td, row, col, prop, value, cellProperties) {
      ...
      tbl = this.HTMLWidgets.find('#' + instance.rootElement.id)
...
Numeric columns are formatted using the [numebro.js](http://numbrojs.com/) library.
...
The `culture` parameter for `hot_col` can be used to change the locale.  See the [numbro.js](http://numbrojs.com/) library for culture options.

```{r}
DF = data.frame(dollar = rnorm(10), euro = rnorm(10), yen = rnorm(10))

rhandsontable(DF * 1000, width = 550, height = 300) %>%
  hot_col("dollar", format = "$0,000.00", culture = "en-US") %>%
  hot_col("euro", format = "0,000.00 $", culture = "de-DE") %>%
  hot_col("yen", format = "$0,000.00", culture = "ja-JP")
```

## Read-Only

The whole table can be set to `readOnly` to prevent the user from making changes.
...
```
* [Table with Search](https://ginberg.shinyapps.io/rhandsontable_search/)
```
shiny::runGitHub("rhandsontable", "jrowen", 
...
Shiny bookmarks willl work for `rhandsontable` with some special handling.  See [this issue](https://github.com/rstudio/shiny/issues/1378) for more details.

## Feedback

Please [file a issue](https://github.com/jrowen/rhandsontable/issues) if you experience any problems with the widget or have feature requests.  Pull requests are also welcome.
